...
bool TerrainClass::GeneratePerlin(ID3D11Device* device, float frequency, float amplitude, int smoothValue)
{
	bool result;
	int index;
	srand(time(NULL));
	unsigned int seed = rand() % 255 + 1;
	PerlinNoise pn(seed);
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].x = (float)i;
			m_heightMap[index].y = pn.noise(m_heightMap[index].x * (frequency * 0.5), m_heightMap[index].y * (frequency * 0.5), m_heightMap[index].z * (frequency * 0.5)) * amplitude;
			m_heightMap[index].z = (float)j;
		}
	}
...
bool TerrainClass::GenerateCirclesAlgorithm(ID3D11Device* device, int iterationsCircle, int displacement, int smoothValue)
{
	bool result;
	int index;
	...
	CirclesAlgorithm(iterationsCircle, displacement);
	Smoothing(smoothValue);
...
bool TerrainClass::GenerateFaulting(ID3D11Device* device, int iterationsFaulting, float displacement, int smoothValue)
{
	bool result;
	int index;
	...
	FaultAlgorithm(iterationsFaulting, displacement);
	Smoothing(smoothValue);
...
bool TerrainClass::GenerateHeightMap(ID3D11Device* device, bool perlin, bool fault, int iterationsFaulting, float displacement)
{
	bool result;
	...
	if (perlin)
	{
		...
		for (int j = 0; j<m_terrainHeight; j++)
		{
			for (int i = 0; i<m_terrainWidth; i++)
			{
				...
				m_heightMap[index].x = (float)i;
				//m_heightMap[index].y = 0.0f;
				m_heightMap[index].y = pn.noise(m_heightMap[index].x / 15, m_heightMap[index].y / 15, m_heightMap[index].z / 15) * 5.0f;
...
void TerrainClass::FaultAlgorithm(int iterations, float displacement)
{
	...
	{
		// random int from 0 to 128
		int x1 = rand() % m_terrainWidth; 
		int y1 = rand() % m_terrainHeight;
		int x2 = rand() % m_terrainWidth;
		int y2 = rand() % m_terrainHeight;

		while (x1 == x2 && y1 == y2)
		{
			// random int from 0 to 1280
			int x1 = rand() % m_terrainWidth; 
			int y1 = rand() % m_terrainHeight;
			int x2 = rand() % m_terrainWidth;
			int y2 = rand() % m_terrainHeight;
		...
		for (int j = 0; j < m_terrainWidth; j++)
		{
			for (int i = 0; i < m_terrainHeight; i++)
			{
				...
				//Determine which side of the line from A(x1, y1) to B(x2, y2) 
				//a point P(x,z) falls into
				float d = (j - x1) * (y2 - y1) - (i - y1) * (x2 - x1);
...
void TerrainClass::CirclesAlgorithm(int iterations, int displacement)
{
	for (int z = 0; z < iterations; z++) 
	{
		int x1 = rand() % m_terrainWidth;                        
		...
		int randRadius = (rand() % 20) + 1;                   
															
		//Loop through the height map and check if a given x and z is within the circle
		//if so modify the height values
		for (int j = 0; j < m_terrainWidth; j++) 
		{
			for (int i = 0; i < m_terrainHeight; i++) 
			{
				...
				//get the value for tx and ty to determine distance from circle center
				int tj = j - x1;                      
				int ti = i - y1;               
				//get the distance from circle center
				float distance = sqrtf((tj*tj) + (ti*ti));  
				float pd = (distance * 2) / randRadius;     
				//check if value is within the circle
				if (fabs(pd) <= 1.0) 
				{         
					//apply circle alorithm to get the new height
					m_heightMap[index].y += (displacement / 2.0) + (cos(pd*3.14)*(displacement / 2.0)); 
				}
			}
		}
	}
...
void TerrainClass::Smoothing(int passes)
{
	...
	for (int i = 0; i < passes; i++)
	{
		...
		for (int j = 0; j < m_terrainHeight; j++)
		{
			for (int i = 0; i < m_terrainWidth; i++)
			{
				...
				index = (m_terrainHeight * j) + i;
				// Set height
				heights[index] = (m_heightMap[(m_terrainHeight * j) + i].y);
		...
		for (int j = 0; j < m_terrainHeight; j++)
		{
			for (int i = 0; i < m_terrainWidth; i++)
			{
				...
				if (numOfAverages > 0)
				{
					m_heightMap[index].y = (averageHeight / numOfAverages);
				}
				else
				{
					m_heightMap[index].y = 0.0f;
...
void TerrainClass::sendData(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
